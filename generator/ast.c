 ...
 * 
 * @param caller_name Name of the calling function for logging purposes.
...
void ast_module_builder_set_name(const char* caller_name, int line_nr, ast_module_builder_t* builder, const char* name){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_name", 0, "Caller name is NULL.");
  if(builder == NULL)
  ...
  if(builder->name_set)
    log_error(caller_name, line_nr, "Module name has already been set to '%s'.", builder->p_current_module->name == NULL ? "<NULL>" : builder->p_current_module->name);
...
void ast_module_builder_set_pin(const char* caller_name, int line_nr, ast_module_builder_t* builder, pin_t pin){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_pin", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_pin", 0, "AST module builder is NULL.");
  
  if(builder->pin_set)
    log_error(caller_name, line_nr, "Module pin has already been set to Port: '%c' Pin: '%d'.", builder->p_current_module->pin.port, builder->p_current_module->pin.pin_number);
...
void ast_module_builder_set_enable(const char* caller_name, int line_nr, ast_module_builder_t* builder, bool enable){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_enable", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_enable", 0, "AST module builder is NULL.");
  
  if(builder->enable_set)
    log_error(caller_name, line_nr, "Module enable has already been set to '%d'.", builder->p_current_module->enable);
...
void ast_module_builder_set_kind(const char* caller_name, int line_nr, ast_module_builder_t* builder, module_kind_t kind){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_kind", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_kind", 0, "AST module builder is NULL.");
  
  if(builder->kind_set)
    log_error(caller_name, line_nr, "Module kind has already been set to '%d'.", builder->p_current_module->kind);
...
void ast_module_builder_set_output_type(const char* caller_name, int line_nr, ast_module_builder_t* builder, gpio_type_t type){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_output_type", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_output_type", 0, "AST module builder is NULL.");
  
  if(builder->p_current_module->kind != MODULE_OUTPUT)
    log_error(caller_name, line_nr, "Cannot set output type for non-output module.");
  if(builder->type_set)
    log_error(caller_name, line_nr, "Output module type has already been set to '%d'.", builder->p_current_module->data.output.type);
...
void ast_module_builder_set_output_pull(const char* caller_name, int line_nr, ast_module_builder_t* builder, gpio_pull_t pull){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_output_pull", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_output_pull", 0, "AST module builder is NULL.");
  
  if(builder->p_current_module->kind != MODULE_OUTPUT)
    log_error(caller_name, line_nr, "Cannot set output pull for non-output module.");
  if(builder->pull_set)
    log_error(caller_name, line_nr, "Output module pull has already been set to '%d'.", builder->p_current_module->data.output.pull);
...
void ast_module_builder_set_output_speed(const char* caller_name, int line_nr, ast_module_builder_t* builder, gpio_speed_t speed){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_output_speed", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_output_speed", 0, "AST module builder is NULL.");
  
  if(builder->p_current_module->kind != MODULE_OUTPUT)
    log_error(caller_name, line_nr, "Cannot set output speed for non-output module.");
  if(builder->speed_set)
    log_error(caller_name, line_nr, "Output module speed has already been set to '%d'.", builder->p_current_module->data.output.speed);
...
void ast_module_builder_set_output_init(const char* caller_name, int line_nr, ast_module_builder_t* builder, gpio_init_t init){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_output_init", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_output_init", 0, "AST module builder is NULL.");
  
  if(builder->p_current_module->kind != MODULE_OUTPUT)
    log_error(caller_name, line_nr, "Cannot set output init for non-output module.");
  if(builder->init_set)
    log_error(caller_name, line_nr, "Output module init has already been set to '%d'.", builder->p_current_module->data.output.init);
 ...
 */
void ast_module_builder_set_output_active_level(const char* caller_name, int line_nr, ast_module_builder_t* builder, level_t active_level){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_output_active_level", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_output_active_level", 0, "AST module builder is NULL.");
  
  if(builder->p_current_module->kind != MODULE_OUTPUT)
    log_error(caller_name, line_nr, "Cannot set output active level for non-output module.");
  if(builder->active_level_set)
    log_error(caller_name, line_nr, "Output module active level has already been set to '%d'.", builder->p_current_module->data.output.active_level);
...
void ast_module_builder_set_input_pull(const char* caller_name, int line_nr, ast_module_builder_t* builder, gpio_pull_t pull){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_input_pull", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_input_pull", 0, "AST module builder is NULL.");
  
  if(builder->p_current_module->kind != MODULE_INPUT)
    log_error(caller_name, line_nr, "Cannot set input pull for non-input module.");
  if(builder->pull_set)
    log_error(caller_name, line_nr, "Input module pull has already been set to '%d'.", builder->p_current_module->data.input.pull);
 ...
 */
void ast_module_builder_set_input_active_level(const char* caller_name, int line_nr, ast_module_builder_t* builder, level_t active_level){
  if(caller_name == NULL)
    log_error("ast_module_builder_set_input_active_level", 0, "Caller name is NULL.");
  if(builder == NULL)
    log_error("ast_module_builder_set_input_active_level", 0, "AST module builder is NULL.");
  
  if(builder->p_current_module->kind != MODULE_INPUT)
    log_error(caller_name, line_nr, "Cannot set input active level for non-input module.");
  if(builder->active_level_set)
    log_error(caller_name, line_nr, "Input module active level has already been set to '%d'.", builder->p_current_module->data.input.active_level);
